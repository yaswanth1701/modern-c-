...
void AddTitle(const string &title) {
  string add_title = fmt::format("<title> {} </title>", title) cout
                     << "\n <title>" + title + "</title>"
}
...
void AddImage(const string &img_path, float score, bool highlight = false) {
  ...
  if (highlight) {
    ...
    string open_col = fmt::format("<div class='column'>");
  }
  string add_name = fmt::format("<h2>{}</h2>", img_name.str());
  string add_src = fmt::format("<img src='{}' />", img_path);
  string add_score = fmt::format("<p> score= {:.2f} </p>", score);
  string close_col = "</div>" cout << open_col << "\n";
  cout << add_name << "\n";
  cout << add_src << "\n";
  cout << add_score << "\n";
  cout << close_col << "\n";
